...
""")
    ...
    async def on_message(self, message):
        if message.author.id == int(TARGET) and message.author != self.user:
            await message.reply(CUSTOM_MESSAGE, mention_author=True)

async def main():
    global TOKEN, TARGET, CUSTOM_MESSAGE
    ...
    TARGET = input(f"{Fore.GREEN}{Style.BRIGHT}Enter the User you want to reply to's ID: ").strip()
    clear_screen()
